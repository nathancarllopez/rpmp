...
import calculateMeals from "../../business-logic/old/calculateMeals";
    ...
    () =>
      proteinsWithFlavors.reduce(
        ...
        {} as {
          [protein: string]: {
            label: string;
            flavors: { value: string; label: string }[];
          };
        }
      ),
    [proteinsWithFlavors]
...
function useQueryData() {
  ...
  const { data: flavorInfo, error: flavorError } = useSuspenseQuery({
    ...flavorsOptions(),
    select: (data) =>
      data.reduce(
        (info, row) => {
  ...
  const { data: headersInfo, error: headerError } = useSuspenseQuery({
    ...orderHeadersOptions(),
    select: (data) =>
...
//   const { data: pullList, error: pullListError } =
